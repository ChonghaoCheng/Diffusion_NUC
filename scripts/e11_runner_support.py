...
def report_e11(root,config,output):
    devr=_csv(output/"dev_results.csv");devv=_csv(output/"dev_final_validation.csv");tr=_csv(output/"transfer_results.csv");tv=_csv(output/"transfer_final_validation.csv") if (output/"transfer_final_validation.csv").exists() else [];lines=["# E11 mechanism attribution and frozen-policy placement transfer","",f"Executed: {datetime.now().astimezone().isoformat()}","","## DEV attribution","","| scene | F | A | A_root | P |","|---|---|---|---|---|"]
    for sid in config["dev_placements"]:
        cells=[]
        for m in ("F","A","A_root","P"):
            r=next(x for x in devr if x["scene_id"]==sid and x["method"]==m);v=next(x for x in devv if x["scene_id"]==sid and x["method"]==m);cells.append(f"{v['overall_status']}; {r['termination']}; {float(r.get('core_seconds') or 0):.1f}s")
        lines.append(f"| {sid} | "+" | ".join(cells)+" |")
    lines += ["","## TRANSFER","","| scene | k | F | P | A |","|---|---:|---|---|---|"]
    for sid in config["transfer_placements"]:
        for k in config["transfer_on_segment_budgets"]:
            cells=[]
            for m in ("F","P","A"):
                r=next((x for x in tr if x["scene_id"]==sid and int(x["k"])==k and x["method"]==m),None);v=next((x for x in tv if x["scene_id"]==sid and int(x["k"])==k and x["method"]==m),None);cells.append((v or {}).get("overall_status") or (r or {}).get("termination") or "NOT_RUN")
            lines.append(f"| {sid} | {k} | "+" | ".join(cells)+" |")
    lines += ["","## Boundaries","","F is an internal fixed-library baseline. P is a transparent greedy control. Results concern six pose perturbations of the same hemisphere, not cross-surface generalization or published-planner superiority. Missing graph edges are sampled-construction limitations, not physical infeasibility. Acceptance is refined finite sampling, not continuous or hardware certification. Source-run B, G1, FM, hardware, non-spherical geometry, and parameter tuning were NOT_RUN."]
    (output/"report.md").write_text("\n".join(lines)+"\n");write_json(output/"report.checkpoint.json",{"complete":True})
