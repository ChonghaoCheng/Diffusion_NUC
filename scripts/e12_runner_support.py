...
def report_e12(root:Path,config:dict[str,Any],output:Path)->None:
    ...
    gpu_names=sorted({str(x.get("gpu_name")) for x in training.get("workers",{}).values() if x.get("gpu_name")});training_summary=f"Training used root seed {training.get('root_training_seed')} with deterministic per-model streams on {', '.join(gpu_names) if gpu_names else 'the recorded CUDA worker'}; all checkpoints are from update 5000."
