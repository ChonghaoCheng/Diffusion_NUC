...
def merge_teacher_shards(root:Path,config:dict[str,Any],output:Path)->None:
    shard_root=output/"teacher_shards";graphs=[];roots=read_csv(output/"root_events.partial.csv") if (output/"root_events.partial.csv").exists() else [];candidates=[];labels=[];events=[];worker_rows=[]
    ...
    expected=sum(x["split"] in {"TRAIN","VALIDATION"} for x in json.loads((root/config["inputs"]["pose_splits"]).read_text())["scenes"])
    if len(scene_ids)!=expected or len(set(scene_ids))!=expected:raise RuntimeError(f"teacher shards contain {len(scene_ids)} rows / {len(set(scene_ids))} unique scenes; expected {expected}")
    unique_roots={(x.get("scene_id"),x.get("seed_index"),x.get("seed_role")):x for x in roots};roots=sorted(unique_roots.values(),key=lambda x:(x.get("scene_id",""),int(x.get("seed_index",-1))));graphs.sort(key=lambda x:x["scene_id"]);labels.sort(key=lambda x:(x["scene_id"],x["program_hash"]));resource_audit=json.loads((output/"parallel_resource_audit.json").read_text()) if (output/"parallel_resource_audit.json").exists() else None;write_json(output/"teacher_graph_manifest.json",{"graphs":graphs,"large_graphs_published":False,"parallel_workers":len(worker_rows),"resource_audit":resource_audit});write_csv(output/"root_events.csv",roots);write_csv(output/"teacher_candidate_qualification.csv",candidates);write_json(output/"qualified_program_dataset.json",{"labels":labels});write_csv(output/"teacher_events.csv",events);write_csv(output/"teacher_worker_summary.csv",worker_rows);write_json(output/"collect-train-val.checkpoint.json",{"complete":True,"attempted_tasks":expected,"usable_graphs":sum(x.get("status") in {"ready","recombination_limited"} for x in graphs),"qualified_labels":len(labels),"qualified_tasks":len({x["scene_id"] for x in labels}),"parallel_workers":len(worker_rows)})
