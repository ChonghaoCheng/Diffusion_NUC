...
def make_plots(root,config,output,rows):
    ...
    for row in rows:
        ...
        source=np.load(root/row["plot_source"]);p=source["actual_surface_points"];a=source["activity"]; samples=source["quadrature_points"]; counts=source["episode_counts"]
        colours=np.where(counts==0,"#777777",np.where(counts==1,"#70ad47","#d62728"))
        fig=plt.figure(figsize=(6,5));ax=fig.add_subplot(111,projection="3d");ax.scatter(samples[:,0],samples[:,1],samples[:,2],c=colours,s=.7,alpha=.35);ax.plot(p[a,0],p[a,1],p[a,2],c="#1f77b4",lw=.45,label="actual FK ON");
        if np.any(~a):ax.plot(p[~a,0],p[~a,1],p[~a,2],c="#ff7f0e",lw=.7,label="actual FK OFF")
        ax.set_title(f"{row['scene_id']} k={row['k']} {row['method']} whole hemisphere");ax.legend();fig.tight_layout();fig.savefig(directory/f"{row['scene_id']}_k{row['k']}_{row['method']}_coverage.png",dpi=150);plt.close(fig)
        fig,ax=plt.subplots(figsize=(7,2.8));ax.plot(source["sigma5"],lw=.6);ax.axhline(float(config["robot"]["sigma_safe"]),color="r",ls="--",lw=.8);ax.set_xlabel("validation sample");ax.set_ylabel("sigma5");ax.set_title(f"{row['scene_id']} k={row['k']} {row['method']}");fig.tight_layout();fig.savefig(directory/f"{row['scene_id']}_k{row['k']}_{row['method']}_sigma5.png",dpi=150);plt.close(fig)
    anytime=read_csv(output/"anytime.csv"); fig,ax=plt.subplots(figsize=(7,4))
    for key in sorted({(r["scene_id"],r["k"],r["method"]) for r in anytime}):
        values=[r for r in anytime if (r["scene_id"],r["k"],r["method"])==key]; ax.step([float(v["seconds"]) for v in values],[1.0 if v["found"]=="True" else 0.0 for v in values],where="post",label="/".join(key))
    ax.set_xlabel("search seconds");ax.set_ylabel("accepted graph incumbent");ax.set_ylim(-.05,1.05);ax.legend(fontsize=6,ncol=3);fig.tight_layout();fig.savefig(directory/"global_anytime.png",dpi=150);plt.close(fig)
    bank=load_bank(output); fig=plt.figure(figsize=(6,5));ax=fig.add_subplot(111,projection="3d");ports=bank["ports"];ax.scatter(ports[:,0],ports[:,1],ports[:,2],s=4,c="#555555");ax.set_title("Frozen ports; verified cross-port ON connections: 0");fig.tight_layout();fig.savefig(directory/"verified_cross_port_graph.png",dpi=150);plt.close(fig)
