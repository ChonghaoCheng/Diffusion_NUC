...
class _Q3Screen:
    ...
    def __call__(self, edge_ids: tuple[int, ...]) -> dict[str, Any]:
        ...
        checked = evaluate_synchronized_fk_trace(self.robot, trace, transform, np.zeros((1, 3)), np.ones(1), sphere_radius=float(self.config["surface"]["radius_m"]), footprint_radius=float(self.config["coverage"]["footprint_radius_m"]), characteristic_length=float(self.config["robot"]["characteristic_length_m"]))
